	...
	switch(connState)
	{
		case WIFI_CONNECTED:
			...
			if(r->success)
			{
					...
					os_sprintf(data, "http://%s/update?key=%s&field1=%s&field2=%s&field3=%d&status=dev_ip:%d.%d.%d.%d", THINGSPEAK_SERVER, THINGSPEAK_API_KEY, temp, hum, vdd, IP2STR(&ipConfig.ip));
...
void user_init(void)
{
	...
	os_timer_arm(&sleep_timer, 15000, 1); //5s

	DHT22_DEBUG("Start deep sleeping.\n");
}
